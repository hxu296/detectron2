...
class RTTSEvaluator(DatasetEvaluator):
    """
    Evaluate RTTS AP.
    ...
    def evaluate(self, use_07_metric=False, debug=False):
        ...
        self._logger.info(
            "Evaluating {}. "
            "Note that results do not use the official Matlab API.".format("RTTS")
        )

        with tempfile.TemporaryDirectory(prefix="rtts_eval_") as dirname:
...
def rtts_eval(detpath, annopath, imagesetfile, classname, ovthresh=0.5, use_07_metric=False, debug=False):
    ...
    Top level function that does the RTTS evaluation.
